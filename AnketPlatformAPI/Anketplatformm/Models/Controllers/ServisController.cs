...
namespace Anketplatformm.Controllers
{
    public class ServisController : ApiController
    {
        ...
        public SonucModel AnketEkle(AnketModel model)
        {
            ...
            yeni.anketKatId = model.anketKatId;
        ...
        [HttpPost]
        [Route("api/kulfotoguncelle")]
        public SonucModel KulFotoGuncelle(kulFotoModel model)
        {
            Kullanici kul= db.Kullanici.Where(s => s.kulId == model.kulId).SingleOrDefault();
            if(kul == null)
            {
                sonuc.islem = false;
                sonuc.mesaj = "Kayıt Bulunamadı";
            ...
            if (kul.kulFoto != "profil.jpg")
            {
                string yol = System.Web.Hosting.HostingEnvironment.MapPath("~/Dosyalar/" +kul.kulFoto);
                if (File.Exists(yol)) 
                {
                    File.Delete(yol);
                }
            ...
            string dosyaAdi = kul.kulId + model.fotoUzanti.Replace("image/",".");
